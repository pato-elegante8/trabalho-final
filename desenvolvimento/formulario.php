...
.footer {
  ...
  <body>
    <div class="container">
      ...
      <form class="formulario" method="POST" action="obrigado.php">
         <h2>Questionário</h2>


         <?php
include 'conecta.php'; // Certifique-se de que esse arquivo conecta corretamente ao seu banco

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Verifica e recebe os dados do formulário com segurança
    $esportes  = mysqli_real_escape_string($conn, $_POST['esportes'] ?? '');
    $musicas   = mysqli_real_escape_string($conn, $_POST['musicas'] ?? '');
    $cursos    = mysqli_real_escape_string($conn, $_POST['cursos'] ?? '');
    $palestras = mysqli_real_escape_string($conn, $_POST['palestras'] ?? '');
    $workshops = mysqli_real_escape_string($conn, $_POST['workshops'] ?? '');
    $teatros   = mysqli_real_escape_string($conn, $_POST['teatros'] ?? ''); // Corrigido nome e variável

    // Verifica se já existe uma entrada igual
    $verifica = "SELECT * FROM quest WHERE esportes='$esportes' AND teatros='$teatros'";
...
?>
        ...
        <div class="pergunta">
         ...
         <select class="form-select" id="teatros" name="teatro" required>
                            <option value="" selected disabled>Faça a sua escolha</option>
